...
using MaterialManagemenetProject.Core.Service;
using MaterialManagementProject.DAL.Context;
using System;
...
namespace MaterialMangementProject.Service.BaseService
{
    public class ServiceBase<T> : ICoreService<T> where T : CoreEntity
    {
        ...
        {
            return context.Set<T>().Where(x => x.Status == MaterialManagemenetProject.Core.Enum.Status.Active).ToList();
        }
        ...
        {
            T item = GetByID(id);
            item.Status = MaterialManagemenetProject.Core.Enum.Status.Deleted;
            Update(item);
        }
